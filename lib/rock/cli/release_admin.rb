...
module Rock
    module CLI
        ...
        class ReleaseAdmin < Thor
            ...
            desc "update-metabuilds", "This updates the rock -all metabuilds for the requested release"
            option :release, type: :string, doc: "The Release which should be updated, either rock-rc rock-master or one of the current releases", default: nil
            def update_metabuilds
                release_path = Release::ROCK_RELEASE_INFO
                if options[:release] == "rock-rc"
                    release_path[:branch] = options[:release]
                elsif options[:release] == "rock-master" ||  options[:release] == "master"
                    release_path[:branch] = "master"
                elsif options[:release] == nil
                    raise ArgumentError, "please enter a branch to create the metabuild"
                else
                    release_path[:branch] = nil
                    release_path[:tag] = options[:release]
                end

                vcs = Autoproj::VCSDefinition.from_raw(release_path)
                buildconf = Autoproj::Ops::Tools.create_autobuild_package(vcs, "main configuration", config_dir)
                commit_id = buildconf.importer.rev_parse(buildconf, release_path[:tag] || release_path[:branch])


                ## Update the rc-all build
                all_manifest = YAML.load_file Autoproj.manifest.file
                ## Override layout section with all metapackages
                all_manifest['layout'] = manifest.metapackages.collect {|m| m[0] }
                commit = Autoproj::Ops::Snapshot.create_commit(buildconf, "manifest", "Manifest which includes all packages", commit_id) do |io|
                    io.puts "# This file is autogenerated by rock-release admin tool during release generation"
                    io.puts "# do not push the modified contend, use the rock-release tool"
                    io.puts "# if you are not a rock-maintainer and have doubts about howto create"
                    io.puts "# your project, refer http://rock-robotis.org or send a mail to rock-users@dfki.de"
                    io.write all_manifest.to_yaml 
                end

                commit_2 = Autoproj::Ops::Snapshot.create_commit(buildconf, "bootstrap.sh", "bootstrap.sh for this release", commit) do |io|
                    File.open(File.join(buildconf.srcdir,"bootstrap.sh")).readlines.each do |line|
                        if line.include? "BOOTSTRAP_ARGS="
                            io.puts "BOOTSTRAP_ARGS=tag=" + options[:release] + "-all"
                        else
                            io.puts line
                        end
                    end
                end
                buildconf.importer.run_git_bare(buildconf, 'tag', '-f', "#{options[:release]}-all", commit_2)
                buildconf.importer.run_git_bare(buildconf, 'push', '-f', '--tags', buildconf.importer.push_to)
            end
